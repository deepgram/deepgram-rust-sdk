...
- Public enums that mirror server values are `#[non_exhaustive]` and carry an `Unknown` variant that keeps the raw wire value (`FluxResponse::Unknown`, `TurnEvent::Unknown`, `TurnTrigger::Unknown(String)`), so a new server message or value never breaks a deployed client. Do the same for any new response enum, and add a deserialization test in the module's `tests` block.
