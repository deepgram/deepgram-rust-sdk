...
mod serialize_options_tests {
    ...
    #[test]
    fn language_hint() {
        // No hints set → no query params emitted.
        check_serialization(&Options::builder().build(), "");

        // Empty iterator → no query params (would surprise users if it
        // emitted `language_hint=`).
        check_serialization(&Options::builder().language_hint::<_, &str>([]).build(), "");

        // Single hint.
        check_serialization(
            &Options::builder().language_hint(["en"]).build(),
            "language_hint=en",
        );

        // Multiple hints → one repeated query param per hint, in the
        // order they were supplied.
        check_serialization(
            &Options::builder().language_hint(["en", "es", "fr"]).build(),
            "language_hint=en&language_hint=es&language_hint=fr",
        );

        // Calling language_hint twice REPLACES (does not append),
        // matching the doc comment on OptionsBuilder::language_hint.
        check_serialization(
            &Options::builder()
                .language_hint(["en"])
                .language_hint(["es", "fr"])
                .build(),
            "language_hint=es&language_hint=fr",
        );
    }
