...
- Flux speech-to-text (`/v2/listen`): a transport failure now reaches the consumer even when the consumer is behind on responses. Previously, when the bounded response channel (256 items) was full at the moment the worker's write to the socket failed — a `send_data`, `configure`, `force_end_turn`, or `close_stream` reaching a dead peer while your code had not been draining `receive()` — the worker forwarded the terminal error with a blocking send and parked there instead of ending the session; the caller then filled the bounded command channel and parked in `send_data` too, so the call never returned and the error never arrived. What you observe now: the failing send returns `Err`, `receive()` yields the already-buffered responses followed by exactly one `Err` carrying the transport failure, then `None`, and later sends keep failing instead of appearing to succeed. The error now travels through a sender held for that purpose, whose own guaranteed slot means the forward neither waits for room nor discards the error when there is none — a plain non-blocking send on the worker's own sender does discard it, since that is the sender parked on the full channel: in `tests/flux_backpressure_local.rs` it leaves the consumer with 257 responses and zero errors. Present since Flux speech-to-text shipped in 0.8.0.
