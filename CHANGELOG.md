...
- Voice Agent WebSocket client (`agent` feature): `Deepgram::agent()` returns an `Agent` sub-client; `start()` / `start_at_url()` open a live session at `wss://agent.deepgram.com/v1/agent/converse` and return an `AgentHandle` and an `AgentEventStream` (implements `futures::Stream`).
  - **Client controls** (typed `send_*` methods plus binary audio via `send_data`): `Settings`, `UpdateListen`, `UpdateSpeak`, `UpdateThink`, `UpdatePrompt`, `InjectUserMessage`, `InjectAgentMessage` (behaviors `default` / `queue` / `interrupt`), `FunctionCallResponse`, `KeepAlive`, and `ForceEndTurn` (`force_end_turn()`, Flux listen providers only).
  - **Server events** (typed `AgentResponse` variants): `Welcome`, `SettingsApplied`, `ConversationText`, `UserStartedSpeaking`, `AgentThinking`, `FunctionCallRequest`, `FunctionCallCancelled` (cancelled `functions[].{id,name}` — do not respond to these), `AgentStartedSpeaking`, `AgentAudioDone`, `LatencyReport` (per-turn STT/LLM/TTS latencies), `History`, `Error`, `Warning`, `ListenUpdated` / `PromptUpdated` / `SpeakUpdated` / `ThinkUpdated`, `InjectionRefused`, server-side `FunctionCallResponse`. Any event the SDK does not model lands in the `Unknown(serde_json::Value)` catch-all rather than breaking the stream. One known off-spec message: Flux (V2) sessions can emit `{"type":"EndOfTurn","trigger":"model"|"timeout"}`, which is not in the AsyncAPI spec yet and currently arrives as `Unknown`.
  - **Settings**: inline or saved (`Uuid`) agent configs; `listen` providers Deepgram V1 (Nova) and V2 (Flux, with `language_hints` array, `eot_threshold`, `eager_eot_threshold`, `eot_timeout_ms`, `keyterms`); `think` providers OpenAI/Anthropic/AWS Bedrock/Google/Groq with function definitions (including `defer_until_eot` for irreversible actions); `speak` providers Deepgram (`version: v1` Aura and `version: v2` Flux TTS voices with `speed` and `expressivity`), ElevenLabs, Cartesia, OpenAI, AWS Polly; audio I/O config; conversation history/context.
  - **Connection behavior**: the worker uses fair (unbiased) selection between inbound frames and outbound commands so continuous agent audio cannot starve outgoing audio, function responses, or `KeepAlive`; after the session ends (peer close or transport error) every retained `AgentHandle` send fails and the worker exits; if the consumer drops the `AgentEventStream` instead, the worker notices on its next inbound frame or close (on a silent connection sends keep succeeding until then) and then fails sends the same way. A normal WebSocket close (code 1000) ends the event stream without an error item; only abnormal close codes are surfaced as `DeepgramError::WebsocketClose`.
  - **Security**: `start_at_url` requires `wss://` — plain `ws://` is accepted only for loopback hosts (`localhost`, `127.0.0.0/8`, `::1`) so local mock servers work; any other cleartext URL is rejected with an `InsecureAgentUrl` error before the credential is sent. `wss://` sessions connect through the client's shared rustls connector (see `tls`), so the `rustls-tls-native-roots` feature and `Deepgram::tls_config` apply to the Voice Agent exactly as they do to live transcription and text-to-speech, and an untrusted server certificate surfaces as `DeepgramError::UntrustedTlsCertificate`. `Debug` output for `AwsCredentials`, `Endpoint`, and `FunctionEndpoint` redacts AWS keys/tokens and header values.
...
- `Speak::speak_to_file` no longer prints `Audio saved to …` to stdout on success; libraries should not write to stdout. If your program relied on that line, print it yourself after the call returns. Error diagnostics are unchanged.
