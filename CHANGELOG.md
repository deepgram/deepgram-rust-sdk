...
- Voice Agent WebSocket client (`agent` feature): `Deepgram::agent()` returns an `Agent` sub-client; `start()` / `start_at_url()` open a live session at `wss://agent.deepgram.com/v1/agent/converse` and return an `AgentHandle` (send `Settings`, `UpdateSpeak`/`UpdateThink`/`UpdatePrompt`, `InjectUserMessage`/`InjectAgentMessage`, `FunctionCallResponse`, `KeepAlive`, plus binary audio) and an `AgentEventStream` (implements `futures::Stream`) of typed `AgentResponse` events (`Welcome`, `SettingsApplied`, `ConversationText`, `UserStartedSpeaking`, `AgentThinking`, `FunctionCallRequest`, `AgentStartedSpeaking`, `AgentAudioDone`, `History`, `Error`, `Warning`, mid-session `*Updated` acks, and an `Unknown` catch-all). Fully typed `Settings` config with inline or saved (`Uuid`) agent configs, `listen`/`think`/`speak` provider settings (Deepgram, plus OpenAI/Anthropic/AWS Bedrock/Google/Groq think providers and Deepgram/ElevenLabs/Cartesia/OpenAI/AWS Polly speak providers), audio I/O config, and conversation history/context. New examples `simple_agent` and `function_calling` ([#152](https://github.com/deepgram/deepgram-rust-sdk/issues/152)). The REST saved-configuration/variables namespace is deferred (see the execution report).
