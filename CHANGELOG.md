...
- **BREAKING**: `manage::usage::response::Details` now exposes `duration`, `total_audio`, `channels`, and `streams` as `Option<f64>`, `Option<f64>`, `Option<usize>`, and `Option<usize>` instead of `f64`, `f64`, `usize`, and `usize`. Code that read these fields directly (`details.duration + 1.0`, `details.channels`) stops compiling; unwrap the option first and decide deliberately what an absent value means for your accounting. `None` means only that the API omitted the `response.details` meter group — not that the request metered no audio, since Voice Agent records report their duration in a separate `sts_details` object this type does not model, so reading `None` as zero can silently drop real usage. Required because `Usage::list_requests` and `Usage::get_request` were unusable against the live API: the four fields were declared required, the API omits the group for many records, and decoding failed with serde's "missing field duration" on any page containing such a record.
...
- `Usage::list_requests` and `Usage::get_request` failed to decode successful (HTTP 200) responses from the live API. The API omits `duration`, `total_audio`, `channels`, and `streams` from `response.details` for `/v1/read`, `/v1/speak`, `/v2/speak`, and `/v1/agent/converse` records — which report what they meter elsewhere in the payload (`token_details`, `tts_details`, `sts_details`) — and for some Flux speech-to-text (`/v2/listen`) records. Because the model required them, a single such record failed the whole page. Verified against 3,943 live request records covering batch and streaming, succeeded and failed, callback and non-callback, and unresolved (`"response": null`) variants: all decode after the fix, and no other field in the payload was over-constrained.
