...
- **BREAKING**: Management calls (`projects`, `keys`, `members`, `scopes`, `invitations`, `usage`, `billing`) now go to the base URL the client was built with, so a client built with `Deepgram::with_base_url` / `with_base_url_and_api_key` / `with_base_url_and_temp_token` sends them to that host instead of to `https://api.deepgram.com`. Code that relied on management calls reaching the hosted API from a custom-base-URL client must make them on a second client built with `Deepgram::new(api_key)` (or `with_temp_token`), which is pointed at `https://api.deepgram.com`. No public signature changed, and a client built with `Deepgram::new` is unaffected. Cause: every module under `src/manage/` built its URL from a hardcoded `https://api.deepgram.com`, so `with_base_url` was silently ignored for management traffic — a client pointed at a proxy, a private deployment, or a mock server sent its management requests, and the credential attached to them, to the public API with no error. `/v1/listen`, `/v1/speak`, `/v2/listen`, and `/v2/speak` already honored the base URL; the management API is now built the same way (`base_url.join("v1/…")`), including a base URL that carries a path prefix.
