...
- Model-listing endpoints on the management API: `Deepgram::models()` returns a `Models` client with `get_models`, `get_model`, `get_project_models`, and `get_project_model`. `Model`'s `name`, `canonical_name`, `architecture`, `version`, and `uuid` are `Option<String>` to match the Models API, which marks them optional. `manage::models::response::Model` (the metadata record) and `common::options::Model` (the request-time selector) now cross-link in the docs. New example `models`.
