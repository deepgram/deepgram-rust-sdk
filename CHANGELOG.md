...
- **BREAKING**: Management calls (`projects`, `keys`, `members`, `scopes`, `invitations`, `usage`, `billing`) and the token grant (`Auth::grant`, `POST /v1/auth/grant`) now go to the base URL the client was built with, so a client built with `Deepgram::with_base_url` / `with_base_url_and_api_key` / `with_base_url_and_temp_token` sends them to that host instead of to `https://api.deepgram.com`. Code that relied on those calls reaching the hosted API from a custom-base-URL client must make them on a second client built with `Deepgram::new(api_key)` (or `with_temp_token`), which is pointed at `https://api.deepgram.com`. No public signature changed, and a client built with `Deepgram::new` is unaffected. Cause: every module under `src/manage/`, and `Auth::grant`, built its URL from a hardcoded `https://api.deepgram.com`, so `with_base_url` was silently ignored for management and auth traffic — a client pointed at a proxy, a private deployment, or a mock server sent those requests, and the credential attached to them, to the public API with no error. `/v1/listen`, `/v1/speak`, `/v2/listen`, and `/v2/speak` already honored the base URL; the management API and `/v1/auth/grant` are now built the same way (`base_url.join("v1/…")`), including a base URL that carries a path prefix. One thing to check while migrating: a base URL that carries a path prefix must end in a slash (`http://gateway/deepgram/`) to keep it, because the endpoint path is joined on by RFC 3986 relative resolution, which replaces the last segment of a path that does not end in one. That was already true of `/v1/listen` and `/v1/speak`; it now applies to management and auth traffic too, and the `with_base_url*` rustdoc now says so.
...
- `Scopes::get_scope` built its URL with a trailing space (`…/scopes `). Harmless in practice — the URL parser strips trailing spaces — but the request line no longer depends on that.
