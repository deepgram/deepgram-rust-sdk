...
- `DeepgramError::UntrustedTlsCertificate { host, trust, source }`: returned instead of a bare `WsError` when the server certificate's issuer is not in the trust roots. The message ends with the remedy that applies to the trust roots actually in effect: enable the feature, install the CA in the OS store, fix an `SSL_CERT_FILE` whose roots could not be loaded, or adjust the supplied config.
