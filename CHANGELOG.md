...
- Voice Agent WebSocket client (`agent` feature): `Deepgram::agent()` returns an `Agent` sub-client; `start()` / `start_at_url()` open a live session at `wss://agent.deepgram.com/v1/agent/converse` and return an `AgentHandle` and an `AgentEventStream` (implements `futures::Stream`).
  ...
  - **Security**: `start_at_url` requires `wss://` — plain `ws://` is accepted only for loopback hosts (`localhost`, `127.0.0.0/8`, `::1`) so local mock servers work; any other cleartext URL is rejected with an `InsecureAgentUrl` error before the credential is sent. `Debug` output for `AwsCredentials`, `Endpoint`, and `FunctionEndpoint` redacts AWS keys/tokens and header values.
